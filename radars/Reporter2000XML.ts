...
export class Reporter2000XML implements RadarParser {
  ...
  getIncidentsByDate = (xmlIncident: any) => {
    ...
    for (let i = 0; i < drivers.length; i++) {
      ...
      const incident: RadarIncident = new RadarIncident({
        speed: Number(speed),
        license,
        date,
        vehicle: new RadarVehicle({
          brand,
          type,
        }),
      });
  ...
  parse(data: any): Radar {
    const json: IReporter2000Data = this.toJson(data);
    const incidents: RadarIncident[] = [];
    const localisation = new RadarLocalisation(json.localisation._attributes.loc);
    ...
    for (let i = 0; i < date.length; i++) {
      const incidentsFromDate: RadarIncident[] = this.getIncidentsByDate(date[i]);
